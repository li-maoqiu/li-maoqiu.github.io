...
let dy1js="<meta name='viewport' content='width=device-width, initial-scale=1'>",wzjm="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",次数=0,制,乱,示,锁,临时,index_dy1js="",dwb="",dwd="0";
...
if(typeof(qm)!="undefined"){for(let i=0;qm.length>i;i++){
    ...
    if(qm[i][0]=="hr"){if(!qm[i][1]){qm[i][1]=""}if(!qm[i][2]){qm[i][2]=""}index_dy1js+="<a href='#"+qm[i][1]+"-"+qm[i][2]+"'>"+qm[i][1]+" "+qm[i][2]+"</a><br>"}
}乱=qm;临时="qm不随机";qm_随机()}
function qm_随机(){
    ...
    if(qm_输入>0){
        ...
        for(let i2=0;qm_输入>i2;i2++){
            if(乱.length==i){i=0;dy1js+="<hr class='hr0'>";打()}
        ...
        for(let i=0;乱.length>i;i++){
            ...
            if(乱[i][0]!="hr" && 乱[i][0]+乱[i][1]+乱[i][2]!=""){dy1js+="<tr><td onclick=\"复(制='"+乱[i][0]+"')\">"+乱[i][0]+"</td><td>"+乱[i][1]+"</td><td>"+乱[i][2]+"</td></tr>"}
        }dy1js+="</table>"}
...
const _执行a={
    ...
    "复制":()=>{dy1js+="<a onclick=\"复(制='"+临时0[2]+"')\">"+临时0[1]+"</a>"},
    "图片":()=>{dy1js+="<a onclick=\"复(制='"+临时0[1]+"')\" href='"+子页面[1]+"2/"+临时0[1]+".jpg' target='_blank'><img src='"+子页面[1]+"2/"+临时0[1]+".jpg' width="+临时0[2]+" "+临时0[3]+" class='bdtp'></a>"},
    "本地图片":()=>{dy1js+="<a onclick=\"复(制='"+临时0[1]+"')\" href='"+子页面[1]+统一[0]+临时0[1]+".jpg' target='_blank'><img src='"+子页面[1]+统一[0]+临时0[1]+".jpg' onerror=\"this.src='"+子页面[1]+"2/0.jpg';this.onerror=null;\" width="+临时0[2]+" "+临时0[3]+" class='bdtq'></a>"},
    ...
    "t":()=>{if(typeof(ts)!="undefined"){if(临时0[1]=="0"){ts[临时0[2]]=临时0[3];if(临时0[4]){临时0=("t,"+临时0[4]).split(",")}}if(临时0[2]&&临时0[2]>-1){if(临时0[2]=="1"){dy1js+=ts[临时0[1]]}}else{if(!ts[临时0[1]]){ts[临时0[1]]="（条目不存在）"}dy1js+="<div class='提'>";if(临时0[2]){if(临时0[2][0]!="#"){临时0[2]=临时0[2]+".html"}dy1js+="<a href='"+临时0[2]+"'>"+临时0[1].split("_")[0]+"</a>"}else{dy1js+=临时0[1].split("_")[0]}dy1js+="<span class='示'>"+ts[临时0[1]]+"</span></div>"}}else{dy1js+=临时0[1]+"<small>(ts缺失)</small>"}},
    ...
    "h1":()=>{if(临时0[2]){}else{临时0[2]=临时0[1]}dy1js+="<a name='"+临时0[2]+"'></a><"+临时0[0]+">"+临时0[1]+"</"+临时0[0]+">";index_dy1js+="<a href='#"+临时0[2]+"'>";if(临时0[0]=="h2"){index_dy1js+="<h3>"+临时0[1]+"</h3>"}else if(临时0[0]=="h3"){index_dy1js+="　<b>"+临时0[1]+"</b>"}else if(临时0[0]=="h4"){index_dy1js+="　"+临时0[1]}else if(临时0[0]=="h5"){index_dy1js+="　<small>"+临时0[1]+"</small>"}else{index_dy1js+=临时0[1]}index_dy1js+="</a><br>"},
    "h2":"h1","h3":"h1","h4":"h1","h5":"h1","h6":"h1",
    "zk":()=>{if(!临时0[2]){临时0[2]=临时0[1]}dy1js+="<a onclick=\"dw(dwb='"+临时0[2]+"',dwd='"+临时0[3]+"')\">"+临时0[1]+"</a>"},
    "http":()=>{dy1js+="<a href='"+临时0[0]+"://"+临时0[1]+"'>"+临时0[2]+"</a>"},
    "https":"http",
    "#":()=>{if(!临时0[2]){临时0[2]=临时0[1]}dy1js+="<a href='#"+临时0[1]+"'>"+临时0[2]+"</a>"},
    ...
    "更多":(i)=>{if(!临时0[2]){临时0[2]="更多"}dy1js+="<span id='隐藏"+i+"' style='display:none;'><fieldset><legend>"+临时0[2]+"</legend>"+临时0[1]+"</fieldset></span><a id='显示"+i+"' onclick=\"显示('"+i+"')\">"+临时0[2]+"▿</a>"},
    "代码":()=>{if(代码[临时0[1]]){dy1js+=代码[临时0[1]]}else{alert(临时0[1]+"不存在")}},
    "a":()=>{if(!临时0[2]){临时0[2]=临时0[1]}if(临时0[2]=="0"){临时0[2]=""}dy1js+="<a name='"+临时0[1]+"'></a>"+临时0[2]},
...
for(let i=1;临时.length>i;i++){
    ...
    if(乱=="a"){dy1js+="<br class='a'><br class='a'><br class='a'><br class='a'><br class='a'>"}
